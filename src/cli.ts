...
export async function runCli() {
  ...
  const BTC_TEMP_CHECK_SHORTCUT = [
    ...
    '1) root BTC.md',
    '2) root SOUL.md',
    '3) .dexter/HEARTBEAT.md if it exists, otherwise docs/HEARTBEAT.example.md',
    '4) fund config / AUM context if available',
    '5) any current BTC / IBIT exposure information I provided',
    '',
    'Then fetch and compare recent performance for:',
    '- BTC-USD',
    '- GLD',
    '- SPY',
    '- IBIT',
    'Use at minimum 7d, 30d, and quarter-to-date when practical.',
    '',
    'Run AIHF as a one-ticker proxy disagreement check on IBIT only.',
    ...
    '- no-action but watch',
    '',
    'The final answer must explicitly separate:',
    '- doctrine',
    '- regime',
    '- concentration',
    '- second-system disagreement',
    '',
    'Save the result with save_report to BTC-TEMP-CHECK-YYYY-MM-DD.md and return the saved filename plus a 3-bullet summary.',
  ].join('\n');
