...
export async function runCli() {
  ...
  const BTC_TEMP_CHECK_SHORTCUT = [
    'Run a BTC core-position temperature check.',
    '',
    'Invoke skill "btc-temp-check".',
    '',
    'Default mode: fast diagnostic.',
    '',
    'Read only the minimum context:',
    '1) root BTC.md',
    '2) root SOUL.md',
    '3) fund config / AUM context if available',
    '4) latest BTC temp check file in .dexter/temp-checks/ if one exists',
    '',
    'Then do one benchmark pass only. Fetch exact total returns for:',
    '- BTC-USD',
    '- GLD',
    '- SPY',
    '- IBIT',
    'Use at minimum 7d, 30d, and quarter-to-date.',
    '',
    'Then fetch BTC ETF flow from one dedicated source only (CoinGlass-style page/API preferred):',
    ...
    '',
    'Fail fast rules:',
    '- If exact GLD/SPY 7d/30d/QTD returns are missing after the first pass, return "incomplete diagnostic".',
    '- If dedicated BTC ETF flow data is not retrievable on the first attempt, return "incomplete diagnostic".',
    '- In incomplete mode, do not run AIHF.',
    '- In incomplete mode, do not run X sentiment.',
    '- In incomplete mode, do not force one of the five actions.',
    '- In incomplete mode, do not save a full memo by default.',
    '- Keep incomplete output extremely short.',
    '',
    'Only if base data is complete enough, run AIHF as a one-ticker proxy disagreement check on IBIT only.',
    'Do not read PORTFOLIO.md for that step.',
    'Treat AIHF-on-IBIT as a second-system calibration signal, not doctrine.',
    '',
    'Only for complete runs, return exactly one decision:',
    '- hold',
    '- trim',
    '- hedge',
    '- prepare to add',
    '- no-action but watch',
    '',
    'For complete runs, keep the answer compact: doctrine, regime, concentration, BTC ETF flow, AIHF.',
    'If a prior BTC temp check exists, name the file and give one-line delta.',
    'Save with save_report only if the run is complete enough to be worth saving.',
  ].join('\n');
...
- Keep this advisory only; do not auto-modify portfolios`,
    ...
    // '/btc-temp-check': BTC_TEMP_CHECK_SHORTCUT, // disabled: data pipeline cannot support this yet — see docs/PRD-BTC-TEMP-CHECK.md
...
Do not place any trades.`,
  ...
  const SHORTCUT_ALIASES: Record<string, string> = {
    '/LFG': '/full-loop',
    '/retro': '/close-loop',
    // '/btc': '/btc-temp-check', // disabled: see docs/PRD-BTC-TEMP-CHECK.md
