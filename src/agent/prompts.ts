 ...
 * User file at .dexter/portfolios/PORTFOLIO.md. Used for gap analysis and rebalance context.
 */
export async function loadPortfolioDocument(): Promise<string | null> {
  const userPath = dexterPortfolioPath('PORTFOLIO.md');
  try {
    return await readFile(userPath, 'utf-8');
  } catch {
    return null;
...
When working with .dexter/portfolios/PORTFOLIO-HYPERLIQUID.md or HIP-3 / on-chain portfolio queries, use this thesis:
${soulHLContent}
` : ''}
